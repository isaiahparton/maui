...
do_numeric_field :: proc(info: Numeric_Field_Info($T), loc := #caller_location) -> (res: Numeric_Field_Result(T)) {
	...
	if self, ok := do_widget(hash(loc), {.Draggable, .Can_Key_Select}); ok {
		...
		if (.Should_Paint in self.bits) {
			paint_rounded_box_corners_fill(self.box, ui.style.rounding, ui.style.rounded_corners, ui.style.color.base[1])
			if .Focused in self.state {
				paint_rounded_box_corners_stroke(self.box, ui.style.rounding, 2, ui.style.rounded_corners, ui.style.color.accent[1])
			}
		}
		// Get text origin
		text_origin: [2]f32 = {self.box.high.x - ui.style.layout.widget_padding, (self.box.low.y + self.box.high.y) / 2}
		// Draw suffix
		if text, ok := info.suffix.?; ok {
			size := paint_text(text_origin, {text = text, font = ui.style.font.content, size = ui.style.text_size.field}, {align = .Right, baseline = .Middle}, ui.style.color.base_text[0])
			text_origin.x -= size.x
		}
		// Main text
		text_res := paint_interact_text(text_origin, self, &ctx.typing_agent, {text = text, font = ui.style.font.content, size = ui.style.text_size.field}, {align = .Right, baseline = .Middle, clip = self.box}, {read_only = true}, ui.style.color.base_text[1])
		text_origin.x = text_res.bounds.low.x
		// Draw prefix
		if text, ok := info.prefix.?; ok {
			paint_text(text_origin, {text = text, font = ui.style.font.content, size = ui.style.text_size.field}, {align = .Right, baseline = .Middle, clip = self.box}, ui.style.color.base_text[0])
		}
		// Value manipulation
		if (.Focused in self.state) {
			// Get the number info
...
do_number_input :: proc(info: Number_Input_Info($T), loc := #caller_location) -> (new_value: T) {
	...
	if self, ok := do_widget(hash(loc), {.Draggable, .Can_Key_Select}); ok {
		...
		// Background
		if .Should_Paint in self.bits {
			paint_rounded_box_corners_fill(box, ui.style.rounding, ui.style.rounded_corners, ui.style.color.base[1])
			if .Focused in self.state {
				paint_rounded_box_corners_stroke(box, ui.style.rounding, 2, ui.style.rounded_corners, ui.style.color.accent[1])
			}
		}
		// Do text interaction
		...
		if state >= {.Focused} {
			...
			text_res = paint_interact_text(
				...
				self,
				&ctx.typing_agent, 
				{text = string(buffer[:]), font = ui.style.font.label, size = ui.style.text_size.label},
			...
			text_res = paint_interact_text(
				...
				self,
				&ctx.typing_agent, 
				{text = text, font = ui.style.font.label, size = ui.style.text_size.label},
				{align = text_align, baseline = .Middle, clip = self.box},
				{},
				ui.style.color.base_text[1],
			)
