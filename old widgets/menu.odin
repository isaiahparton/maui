...
do_menu :: proc(info: Menu_Info, loc := #caller_location) -> (active: bool) {
	...
	if self, ok := do_widget(shared_id); ok {
		...
		if .Should_Paint in self.bits {
			paint_rounded_box_corners_fill(self.box, ui.style.rounding, ui.style.rounded_corners, alpha_blend_colors(alpha_blend_colors(ui.style.color.substance[1], ui.style.color.substance_hover, hover_time), ui.style.color.substance_click, press_time))
			paint_label_box(info.label, self.box, ui.style.color.base_text[1], info.align.? or_else .Middle, .Middle)
		}
		...
		if res, ok := begin_attached_layer({
			...
			shadow = Layer_Shadow_Info{
				offset = 0,
				roundness = ui.style.rounding,
			},
		}); ok {
			paint_rounded_box_fill(res.self.box, ui.style.rounding, ui.style.color.base[1])
			active = true
		}
...
do_submenu :: proc(info: Menu_Info, loc := #caller_location) -> (active: bool) {
	...
	if self, ok := do_widget(shared_id); ok {
		...
		// Paint
		if .Should_Paint in self.bits {
			paint_box_fill(self.box, fade(ui.style.color.accent[0], hover_time))
			// Paint label
			label_box := self.box
			cut_box_left(&label_box, height(label_box))
			label_color := blend_colors(ui.style.color.base_text[0], ui.style.color.accent[0], hover_time)
		...
		if res, ok := begin_attached_layer({
			...
			paint_box_fill(res.self.box, ui.style.color.base[1])
			active = true
		}
...
_do_submenu :: proc(active: bool) {
	...
	if active {
		end_attached_layer({
			mode = .Hover,
			stroke_color = ui.style.color.accent[1],
		}, current_layer())
	}
}
...
do_option :: proc(info: Option_Info, loc := #caller_location) -> (clicked: bool) {
	...
	if self, ok := do_widget(hash(loc)); ok {
		...
		// Painting
		if .Should_Paint in self.bits {
			paint_rounded_box_corners_fill(self.box, ui.style.rounding, ui.style.rounded_corners, fade(ui.style.color.substance[1], hover_time))
			label_color := blend_colors(ui.style.color.base_text[1], ui.style.color.substance_text[1], hover_time)
			label_box := self.box
