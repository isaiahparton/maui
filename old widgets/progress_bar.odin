	...
	if self, ok := do_widget(hash(loc)); ok {
		...
		if .Should_Paint in self.bits {
			...
			if src, ok := atlas_get_ring(&ctx.painter.atlas, 0, radius); ok {
				...
				paint_textured_box(ctx.painter.atlas.texture, src_left, {self.box.low, {self.box.low.x + half_width, self.box.high.y}}, ui.style.color.substance[0])
				paint_textured_box(ctx.painter.atlas.texture, src_right, {{self.box.high.x - half_width, self.box.low.y}, self.box.high}, ui.style.color.substance[0])
				if self.box.high.x > self.box.low.x + size.y {
					paint_box_fill({{self.box.low.x + radius, self.box.low.y}, {self.box.high.x - radius, self.box.high.y}}, ui.style.color.substance[0])
				}
			}
			progress_box: Box = {self.box.low, {self.box.low.x + width(self.box) * clamp(info.time, 0, 1), self.box.high.y}}
			paint_clipped_pill_fill_h(self.box, progress_box, ui.style.color.accent[0])
		}
