...
toggle_switch :: proc(ui: ^UI, info: Toggle_Switch_Info, loc := #caller_location) -> Generic_Widget_Result {
	...
	paint_rounded_box_fill(ui.painter, self.box, ui.style.rounding, blend_colors(data.how_on, ui.style.color.substance, ui.style.color.accent))

	s := width(self.box) / 2
	slider_box := shrink_box(move_box(get_box_left(self.box, s), {s * ease.circular_in_out(data.how_on), 0}), 2)
	paint_rounded_box_fill(ui.painter, slider_box, ui.style.rounding, ui.style.color.background)
