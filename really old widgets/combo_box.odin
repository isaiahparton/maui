...
do_strings_menu :: proc(info: Strings_Menu_Info, loc := #caller_location) -> (new_index: int, changed: bool) {
	...
	if self, ok := do_widget(shared_id); ok {
		...
		if .Should_Paint in self.bits {
			...
			paint_label_box(info.items[info.index], self.box, ui.style.color.base_text[1], .Middle, .Middle)
		}
		...
		if .Menu_Open in self.bits {
			...
			if layer, ok := do_layer({
				...
				paint_rounded_box_fill(layer.box, ui.style.rounding, ui.style.color.base[1])
...
do_enum_menu :: proc(info: Enum_Menu_Info($T), loc := #caller_location) -> (new_value: T, changed: bool) {
	...
	if self, ok := do_widget(shared_id); ok {
		...
		if .Should_Paint in self.bits {
			paint_rounded_box_corners_fill(self.box, ui.style.rounding, ui.style.rounded_corners, alpha_blend_colors(alpha_blend_colors(ui.style.color.substance[1], ui.style.color.substance_hover, hover_time), ui.style.color.substance_click, press_time))
			paint_label_box(reflect.enum_string(info.value), self.box, ui.style.color.base_text[1], .Middle, .Middle)
		}
		// Begin layer if expanded
		if .Menu_Open in self.bits {
			index: int
			...
			if layer, ok := do_layer({
				...
				defer ui.style.rounded_corners = prev_rounded_corners
...
do_combo_box_option :: proc(text: string, index: int) -> (clicked: bool) {
	...
	if w, ok := do_widget(hash(index + 1)); ok {
		...
		if .Should_Paint in w.bits {
			paint_rounded_box_corners_fill(w.box, ui.style.rounding, ui.style.rounded_corners, fade(ui.style.color.substance[1], hover_time))
			// Paint label
			paint_label_box(text, w.box, blend_colors(ui.style.color.base_text[1], ui.style.color.substance_text[1], hover_time), .Middle, .Middle)
		}
