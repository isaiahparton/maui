...
do_toggle_button :: proc(info: Toggle_Button_Info, loc := #caller_location) -> (clicked: bool) {
	...
	if self, ok := do_widget(hash(loc)); ok {
		...
		if .Should_Paint in self.bits {
			...
			// Body
			color := info.color.? or_else (ui.style.color.accent[1] if info.state else ui.style.color.substance[1])
			if info.state {
				...
				paint_rounded_box_corners_fill(self.box, ui.style.rounding, ui.style.rounded_corners, fade(ui.style.color.base_hover, hover_time))
				paint_rounded_box_corners_fill(self.box, ui.style.rounding, ui.style.rounded_corners, fade(ui.style.color.base_click, press_time))
			}
			// Label
			paint_label_box(info.label, self.box, ui.style.color.base_text[1], .Middle, .Middle)
		}
		// Hover
...
do_enum_toggle_buttons :: proc(value: $T, loc := #caller_location) -> (new_value: T) {
	...
	for member, i in T {
		push_id(int(member))
			...
			if horizontal {
				if i == 0 {
					ui.style.rounded_corners += {.Top_Left, .Bottom_Left}
				} else if i == len(T) - 1 {
					ui.style.rounded_corners += {.Top_Right, .Bottom_Right}
				}
			} else {
				if i == 0 {
					ui.style.rounded_corners += {.Top_Left, .Top_Right}
				} else if i == len(T) - 1 {
					ui.style.rounded_corners += {.Bottom_Left, .Bottom_Right}
				}
			}
